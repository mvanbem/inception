...
//! # Bitfield Structs
//!
//! The [`bitfield!`](crate::bitfield) macro generates bitfield structs. Bitfield structs are
//! declared with a sequence of fields, but unlike regular Rust structs those fields are not
//! directly exposed. Instead, they are stored in statically packed ranges of bits in an underlying
//! integer type, and are accessible only by value through methods that perform the necessary
...
//!
//! Generated structs with primitive integer underlying types are particularly well suited for
//! memory-mapped I/O and foreign function interface bindings because they have no forbidden bit
//! patterns.
//!
//! # Bitfield Packing
//!
//! Bitfields occupy contiguous ranges of bits in the underlying type and are tightly packed in
//! declaration order. Every underlying bit is covered by one bitfield. The `..` shorthand for a
//! flexible reserved bitfield may be convenient to ensure every bit is covered.
//!
//! Packing begins with the first declared bitfield at either the least or most significant bit. The
//! packing direction is determined by the mandatory [`#[lsb_first]` or `#[msb_first]` struct
...
//!
//! Bitfields of any width may declare a user-defined accessor type, which must provide the
//! following methods:
//!
//! ```ignore
...
//!
//! All generated bitfield struct types [provide these methods](#underlying-type-conversion) and are
//! valid accessor types for bitfields of the same width.
//!
//! # Bitfield Struct API
//!
//! ## Trait Implementations
//!
//! All generated types invoke the [`Clone`], [`Copy`], [`Debug`], [`PartialEq`], [`Eq`], and
//! [`Hash`] derive macros.
//!
...
//!
//! Generated types with narrow integer underlying types also provide these convenience methods for
//! the nested primitive type:
//!
//! ```ignore
...
//! ## Bitfield Accessors
//!
//! Any methods generated for a bitfield use the visibility specifier from the [bitfield
//! declaration](crate::bitfield#bitfields).
//!
//! A bitfield is _reserved_ if its name starts with an underscore; otherwise it is _defined_.
//! Reserved bitfields do not generate accessor methods.
//!
//! All defined bitfields provide a getter, two functional setters, and two mutating setters:
//!
//! ```ignore
//! const fn        B(self                             ) -> T;
...
//!
//! Bitfields with narrow integer accessor types also provide convenience methods for the nested
//! primitive type.
//!
...
//! ```
//!
//! where `B` is the bitfield's name, `N` is the nested primitive type's width, and `T` is the
//! nested primitive type.
//!
//! The functional setters have variants that compose each of the narrow integer type constructors
//! with the operation:
//!
//! ```ignore
...
//!
//!        fn modify_B_uN          (&mut self, f: impl FnOnce(T) -> T) -> Result<()>;
